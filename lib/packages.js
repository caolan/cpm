...
exports.clone = function (ins, id, dir, callback) {
    couchdb.get(ins, id, function (err, pkg) {
        if (err) return callback(err);
                ...
                async.apply(async.forEach, pkg.cpm.files, function (f, cb) {
                    ...
                    if (type == 'attachments') {
                        utils.ensureDir(path.dirname(fullpath), function (err) {
                            ...
                            couchdb.download(ins, path.join(id, f), fullpath, cb);
                        });
                    }
                    else {
                        ...
                        var val;
                        if (type == 'modules') {
                            try { val = utils.getPropertyPath(pkg, p); }
                            catch (e) { return cb(e); }
                        }
                        if (type == 'properties') {
                            val = pkg.cpm.properties_files[f];
                        }
                        if (type == 'templates') {
                            var pkgs = {'pkg':pkg};
                            try { val = modules.require({}, pkgs, '/pkg', f); }
                            catch (e) { return cb(e); }
                        }
                        ...
                        else cb(new Error('Could not find content for: ' + f));
                    }
                }),
                function (cb) {
                    // write the package.json file, using 4 spaces for indentation
