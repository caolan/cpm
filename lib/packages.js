 ...
 * Location can by of type 'url', 'file' or 'repository'.
 *
 * @param {String} loc
 * @return {String}
 * @api public
...
exports.locationType = function (loc) {
    ...
    if (loc.indexOf('/') != -1) return 'file';
    return 'repository';
};

/**
 * Load package from either a url, repository or the filesystem.
 *
...
exports.load = function (settings, loc, callback) {
    logger.debug('load package:', loc);
    switch (exports.locationType(loc)) {
        case 'file':
            logger.debug('location type:', 'file');
            exports.loadLocal(loc, callback);
            break;
        case 'repository':
            logger.debug('location type:', 'repository');
            var p = repository.splitLocation(loc);

            var ins = repository.resolveInstance(settings, '');
            try { instance.validateInstance(ins); }
            catch (e) { return callback(e); }

            repository.getPackage(ins, p.name, p.version, callback);
            break;
        case 'url':
            logger.debug('location type:', 'url');
            var ins = instance.resolveInstance(settings, loc);
            try { instance.validateInstance(ins); }
            catch (e) { return callback(e); }
            couchdb.get(ins, ins.doc, callback);
            break;
        default:
            callback(new Error('Unknown package location type'));
    }
};

/**
 * Load package metadata from either a url, repository or the filesystem.
 *
 * @param {Object} settings
 * @param {String} loc
 * @param {Function} callback
...
exports.loadMetadata = function (settings, loc, callback) {
    switch (exports.locationType(loc)) {
        case 'file':
            utils.readJSON(path.join(loc, 'package.json'), callback);
            break;
        default:
            exports.load(settings, loc, function (err, doc) {
...
exports.getNameAndVersion = function (settings, loc, callback) {
    switch (exports.locationType(loc)) {
        case 'repository':
            ...
            callback(null, {name: pkg, version: version});
            break;
        default:
            exports.loadMetadata(settings, loc, function (err, meta) {
                if (err) return callback(err);
...
exports.loadLocal = function (d, callback) {

    // change into project directory, all paths are then relative to there
    try { process.chdir(d); }
    catch (e) { return callback(e); }
    d = '.';
...
exports.getInstance = function (settings, loc) {
    var ins;
    switch (exports.locationType(loc)) {
        case 'repository':
            ins = repository.resolveInstance(settings, '');
            break;
        case 'url':
            ins = instance.resolveInstance(settings, loc);
            break;
        default:
            break;
    }
    instance.validateInstance(ins);
    return ins;
};
...
exports.clone = function (settings, loc, dir, callback) {
    try { var ins = exports.getInstance(settings, loc); }
    catch (e) { return callback(e); }

    exports.load(settings, loc, function (err, pkg) {

        // default to using package name as destination directory
        dir = dir || pkg.package.name;
        ...
        fs.mkdir(dir, 0755, function (err) {
            ...
            async.parallel([
                ...
                async.apply(async.forEach, pkg.cpm.files, function (f, cb) {
                    propertyToFile(ins, ins.doc, dir, pkg, f, cb);
                }),
