        ...
        function createTasks(arr, fn) {
            ...
            if (arr instanceof Array) {
                arr.forEach(function (p) {
                    tasks.push(async.apply(fn, settings, d, p, _design));
                });
            }
            else tasks.push(async.apply(fn, settings, d, arr, _design));
        }
...
exports.loadModules = function (settings, dir, p, _design, callback) {
    exports.loadFiles(dir, p, _design, {
        filter: function (filename) {
            return /\.js$/.exec(filename);
        }
 ...
 *
 * Obeys ignore regular expressions from settings when loading files.
 *
 * @param {Object} settings
 * @param {String} dir
 * @param {String} p
...
exports.loadTemplates = function (settings, dir, p, _design, callback) {
    var ignores = (settings.ignore || []).map(function (pattern) {
...
exports.loadAttachments = function (settings, dir, p, _design, callback) {
    ...
    utils.descendants(fullpath, function (err, files) {
        ...
        files.forEach(function (f) {
            var ignore = ignores.some(function (re) {
                return re.test(f);
            });
            if (!ignore) {
    ...
    utils.ensureDir(path.dirname(output), function (err) {
        ...
        switch (transports.locationType(loc)) {
            case 'db':
                ...
                couchdb(ins).download('', output, callback);
                break;
            case 'filesystem':
                // cp files
